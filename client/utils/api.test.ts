...
import { apiFetch, setClerkTokenGetter, UnauthorizedError } from './api.js';
...
describe('apiFetch clerk token', () => {
  ...
  it('retries a 401 once after forcing a Clerk token refresh', async () => {
    ...
    vi.stubGlobal('fetch', async (_url: string, init?: RequestInit) => {
      ...
      return new Response('{"builds":[]}', { status: 200 });
    });
  ...
  it('does not retry 401 when Clerk has no session token', async () => {
    ...
    await expect(apiFetch('/api/builds')).rejects.toBeInstanceOf(UnauthorizedError);
