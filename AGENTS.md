...
- **Encrypted `.env.development` / `.env.production` files.** Decrypt with `pnpm dotenvx decrypt -f .env.development` (requires `DOTENV_PRIVATE_KEY_DEVELOPMENT` env var). For Armory, the Codex key may not match — use a plain `.env` with real Clerk keys instead. Run with `node --env-file=.env` so plain values take precedence.
